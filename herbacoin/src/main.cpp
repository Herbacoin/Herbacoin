...
#if defined(NDEBUG)
# error "Herbacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xcad62306918ed1dbe3a2ebd14890f706f5736bf2c339b03b44745386ef2ca8a5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Herbacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Herbacoin Signed Message:\n";
...
{
    // Herbacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Herbacoin: 420k blocks in ~2 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Herbacoin: 3.5 days
static const int64 nTargetSpacing = 4.2 * 60; // Herbacoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Herbacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Herbacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6750a52829a69d2eeb028b52fd1fa78617272b5a6df2a27405f3435bd0fd3b36");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Herbacoin 28/05/2021 THe Genesis can now exist...";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 420 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04793af38baad42ab5bb074972929e0ddc8ef092e6acb4f3a06d1cde18963217d73bb9760ac3c02dce2f34ab5b81e17c53380015ae3e3c7920f26ac146d658167e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1622234843;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085084630;

        if (fTestNet)
        {
            block.nTime    = 1622234802;
            block.nNonce   = 386732785;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x5677f2f553a69ef8768613440f66d2532d0df10e6e00ca0ff266d68740818aa2"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc2, 0xb8, 0xdf }; // Herbacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HerbacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HerbacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HerbacoinMiner(CWallet *pwallet)
{
    printf("HerbacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("herbacoin-miner");
    ...
    try { loop {
        ...
        printf("Running HerbacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HerbacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HerbacoinMiner, pwallet));
}
