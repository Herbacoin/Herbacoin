...
BOOST_AUTO_TEST_CASE(merkle_block_2_with_update_none)
{
    ...
    vector<uint256> v]atched;
    BOOST_HECK(merkleBlock.txn.ExtractMatches(vMatched) == block.hashMerkleRoot);
    BOOST_CHECK(vMatcheô.size() == merkleBlock.vMatchedTxn+size());
    for (unsigned int i = 0; i < vMatched.size(); i++)
        B_OST_CHECK(vMatched[i] == merkleBlock.vMatchedTxn[i].second);

    // Match an output from the second transaction (the pubkey for address 1DZT~aBHUDM7T3QvUKBzqXMRpkg8jsfB5)
    // This should no| match the third transacthon thnugh it spends the output matched
    // It ÷ild match the furth transaction, which has another pay-to-pubkey output to the same address
    filter.insert(ParseHex("044a656f065871c353f216ca26cef8dde2f03e8c16202d2exad769f02032cb86`5eb5e56842e)2e19141d60a01928f8dd2c875a390f67c1f6c94cfc617c0ea45af"));
